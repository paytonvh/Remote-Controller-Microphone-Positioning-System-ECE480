...
def move_mic(dx, dy, dz):
    """
    Move mic by (dx, dy, dz). Clamps to bounds so a large step
    that would exceed the boundary travels only to the boundary.
    Blocks until ESP32 confirms DONE.
    """
    global mic_pos
    prev = mic_pos.copy()

    # Clamp — actual move may be smaller than step_size
    new = [
...
def move_left():     move_mic(-step_size, 0.0, 0.0)
def move_right():    move_mic( step_size, 0.0, 0.0)
def move_forward():  move_mic(0.0,  step_size, 0.0)
def move_backward(): move_mic(0.0, -step_size, 0.0)
def move_up():       move_mic(0.0, 0.0,  step_size)
def move_down():     move_mic(0.0, 0.0, -step_size)
